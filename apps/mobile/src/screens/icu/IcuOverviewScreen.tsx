...
// ================================================================
// OFFLINE DEMO MODE
// Used only when the backend is unavailable (for APK/UI review).
// The real backend + WebSocket flow remains unchanged.
// ================================================================

const DEMO_PATIENT_ID =
    'fd8ec570-eb5c-469b-b5e4-ba43f243ed96';

const createDemoVital = (patientId: string): Vital => ({
    patientId,
    heartRate: Math.round(75 + Math.random() * 20),
    systolicBP: Math.round(112 + Math.random() * 18),
    diastolicBP: Math.round(65 + Math.random() * 15),
    spo2: Math.round(96 + Math.random() * 4),
    temperature:
        Math.round((36.5 + Math.random() * 0.7) * 10) / 10,
    respiratoryRate: Math.round(14 + Math.random() * 6),
    glucose: Math.round(90 + Math.random() * 30),
    recordedAt: new Date().toISOString(),
});

const createDemoIcu = (): IcuOverview => ({
    id: ICU_ID,
    ...
    beds: [
        {
            ...
            patient: {
                ...
                lastName: 'Patient',
                status: 'ACTIVE',
                latestVital: createDemoVital(DEMO_PATIENT_ID),
            },
        },
        {
            id: 'demo-bed-002',
...
const IcuOverviewScreen = ({
    ...
    // Generate changing vitals only while the backend is unavailable.
    // When the real backend is available, WebSocket data remains the source of truth.
    useEffect(() => {
        if (!demoMode) {
            return;
        }
